...
export function Card({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-[var(--radius-lg)] border border-border-hairline bg-surface-1 shadow-[var(--shadow-sm)]",
        className
      )}
      {...props}
    />
...
export function CardDescription({ className, ...props }: HTMLAttributes<HTMLParagraphElement>) {
  return <p className={cn("text-[13px] text-text-muted mt-0.5", className)} {...props} />;
}
