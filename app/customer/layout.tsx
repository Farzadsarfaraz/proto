...
export default function CustomerLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // A logged-in influencer/intern landing on a /customer link shouldn't see
    // this portal's data — send them back to the one their account is for.
    if (status === "authenticated" && session && session.portal !== "customer") {
      router.replace(PORTAL_HOME[session.portal]);
    }
  }, [status, session, router]);

  if (status !== "authenticated" || session?.portal !== "customer") {
    return (
